 ...
 * @param {string} payload.sub - Subject (membershipId o email)
 * @param {string} payload.src - Source (es. "enotempo")
 * @param {number} payload.iat - Issued at (timestamp)
 * @param {number} payload.exp - Expiration (timestamp)
 * @param {string} [payload.jti] - JWT ID (nonce per replay protection, generato automaticamente se non fornito)
...
export function createHandoffToken(payload) {
  ...
  // Serializza payload a JSON e codifica in base64url
