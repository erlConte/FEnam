...
export async function completeAffiliation({
  ...
  const logContext = { affiliationId, correlationId: correlationId || 'local' }
  ...
  if (!affiliation) {
    logger.error('[Complete Affiliation] Affiliazione non trovata', logContext)
  ...
  // 4) Aggiorna DB - OPERAZIONE CRITICA: se fallisce, l'affiliazione rimane pending
  let updatedAffiliation
  try {
    logger.info('[Complete Affiliation] Aggiornamento DB con status completed', {
      ...logContext,
    ...
    logger.info('[Complete Affiliation] DB aggiornato con successo', {
      ...logContext,
...
Questa email è stata inviata automaticamente. Si prega di non rispondere.
      ...
      emailSent = true
      logger.info('[Complete Affiliation] Email di conferma inviata', {
        ...logContext,
      })
    } catch (emailError) {
    ...
    logger.warn('[Complete Affiliation] Resend non configurato, email di conferma non inviata', logContext)
...
Questa email è stata inviata automaticamente. Si prega di non rispondere.
      ...
      cardSent = true
      logger.info('[Complete Affiliation] Tessera PDF inviata', {
        ...logContext,
        memberNumber: updatedAffiliation.memberNumber,
      })
    } catch (cardError) {
    ...
    logger.warn('[Complete Affiliation] Resend non configurato, tessera PDF non inviata', logContext)
    ...
    logger.warn('[Complete Affiliation] memberNumber non disponibile, tessera PDF non inviata', logContext)
  }
