 ...
 * @param {RateLimiter} options.limiter - Istanza custom del limiter (opzionale)
...
export async function rateLimit(req, res, options = {}) {
  ...
  if (!allowed) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        `⚠️ [Rate Limit] IP ${ip} ha superato il limite di ${maxRequests} richieste/minuto. Retry after ${retryAfterSeconds}s`
      )
    }
